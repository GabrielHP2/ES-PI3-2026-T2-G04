...
export const startupCatalog = onCall(async (request) => {
    ...
    // Pega no BD os campos especificados no "select" respeitando os filtros do "where"
    const snapshot = await db.collection("startups")
    .select("name", "stage", "tags", "icon", "token_symbol", "investors_count", 
        "short_description", "corporate_structure", "tokens_issued", "current_raised")
    .where("status", "==", "ativa")
    ...
    if (snapshot.empty) {
        ...
        logger.error("Error from startupCatalog: Falha ao buscar dados das startups");
        throw new HttpsError("data-loss", "Falha ao buscar dados das startups");
    }


    let startups: SimplifiedStartup[] = [];

    // Le os dados de cada documento e add ao array de startups
    for (const doc of snapshot.docs) {

        const data = doc.data();
        startups.push(data as SimplifiedStartup);
