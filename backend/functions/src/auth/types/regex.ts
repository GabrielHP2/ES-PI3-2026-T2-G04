...
export const emailRegex: RegExp = RegExp(/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}$/i);

// Deve ter 8-16 caracteres, sem espaços, 1 letra maiuscula e minuscula, 1 numero e 1 caractere especial.
export const passwordRegex: RegExp = RegExp(/^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[^\w\d\s:])([^\s]){8,16}$/);

// Deve estar no formato exato 000.000.000-00.
export const cpfRegex: RegExp = RegExp(/^\d{3}\.\d{3}\.\d{3}-\d{2}$/);

// Deve estar no formato dd/mm/aaaa, dia de 01 a 31, mes de 01 a 12 e ano com 4 digitos.
export const birthDateRegex: RegExp = RegExp(/^(0[1-9]|[1-2][0-9]|3[0-1])\/(0[1-9]|1[0-2])\/(\d{4})$/);
